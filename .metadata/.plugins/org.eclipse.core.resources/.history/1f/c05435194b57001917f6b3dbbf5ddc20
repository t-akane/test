...
public class MyPageDAO {
	...
	throws SQLException{
		...
		String sql="SELECT iit.item_name, ubit.total_price, ubit.total_count, ubit.pay FROM user_buy_item_transaction5 ubit LEFT JOIN item_info_transaction5 iit ON ubit.item_transaction_id=iit.id WHERE ubit.item_transaction_id=? AND ubit.user_master_id=? ORDER BY ubit.insert_date DESC ";
		//↑見直し済み
	...
	throws SQLException{
		...
		String sql="DELETE FROM user_buy_item_transaction5 WHERE item_transaction_id=? AND user_master_id=?";
